...
model1=load_model(cwd +'/HyperColumn1_2_RGB_Norm4_tanh_24it.h5')
...
filesTs_list =  glob.glob(os.path.join(cwd +'/50_images_res_lab2/RGB/', '*.jpg'))
...
for j1 in range(1):    
    ...
    sz0=ima0.shape[0]
    sz1=ima0.shape[1]
    RG=np.zeros((sz0,sz1,2))
    R1 = np.float32(ima0[:,:,0]/255)
    G1 = np.float32(ima0[:,:,1]/255)
    B1 = np.float32(ima0[:,:,2]/255)
    ...
    RG[:,:,1] = (G1-0.5)/0.5
    ima_gray =  np.reshape(L,(1,sz0,sz1,1))
        # train_imgs = np.reshape(RG,(1,sz0,sz1,2))    
    ...
    # ima_gray =  np.reshape(ima_LAB[:,:,0],(1,ima_LAB.shape[0],ima_LAB.shape[1],1))
    # if(Norm==1):
    #     # max1=1
    #     ima_gray=ima_gray/100
    # else:
        # max1=255
    predicted0 = model1.predict(ima_gray,verbose=0)
    ...
    predicted0 = np.reshape(predicted0,(sz0,sz1,bands))
    predicted = (predicted0*0.5) + 0.5
    # L=0.299*R1 + 0.587*G1 + 0.114*B1
    Rr=predicted[:,:,0]
    Gr=predicted[:,:,1]
    # Br=(L - 0.299*Rr - 0.587*Gr)/0.114
    
    rr = np.reshape(Rr,(sz0*sz1,1))
    gr = np.reshape(Gr,(sz0*sz1,1))
    l = np.reshape(L,(sz0*sz1,1))
    br=(l - 0.299*rr - 0.587*gr)/0.114
    ...
    predicted255[:,:,0] = np.round(Rr*255)
    predicted255[:,:,1] = np.round(Gr*255)
    predicted255[:,:,2] = np.round(Br*255)
    cv2.imwrite('C:/work/colorisation/50_images_res_lab2/res.jpg',predicted255)
    ...
    # predicted255 = np.uint8(np.round(predicted[0:ima0.shape[0],0:ima0.shape[1],:]*255))
    # if(j1<10):
    #     cv2.imwrite(cwd +'/aerial_images_01/pred_HyperColumn1_2_LAB2_Norm4_tanh_5it/'+files_name1[j1],predicted255)
